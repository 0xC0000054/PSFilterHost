...
namespace PSFilterHostDll.PSApi
{
    internal sealed class DescriptorSuite : IDisposable
    {
        ...
        private unsafe bool GetKeyProc(PIReadDescriptor descriptor, uint* key, uint* type, int* flags)
        {
            ...
            if (descriptor != PIReadDescriptor.Null)
            {
                ...
                if (type != null)
                {
                    ...
                    *type = item.Value is Dictionary<uint, AETEValue> ? DescriptorTypes.Object : item.Type;
                }
        ...
        private unsafe short GetTextProc(PIReadDescriptor descriptor, IntPtr* data)
        {
            ReadDescriptorState state = readDescriptors[descriptor];
        ...
        private unsafe short GetAliasProc(PIReadDescriptor descriptor, IntPtr* data)
        {
            ReadDescriptorState state = readDescriptors[descriptor];
        ...
        private unsafe short GetObjectProc(PIReadDescriptor descriptor, uint* retType, IntPtr* descriptorHandle)
        {
            ReadDescriptorState state = readDescriptors[descriptor];
        ...
        private unsafe short CloseWriteDescriptorProc(PIWriteDescriptor descriptor, IntPtr* descriptorHandle)
        {
#if DEBUG
            DebugUtils.Ping(DebugFlags.DescriptorParameters, string.Empty);
#endif
        ...
        private short PutScopedObjectProc(PIWriteDescriptor descriptor, uint key, uint type, IntPtr handle)
        {
#if DEBUG
            DebugUtils.Ping(DebugFlags.DescriptorParameters, string.Empty);
#endif
