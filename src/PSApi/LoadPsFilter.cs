...
namespace PSFilterHostDll.PSApi
{
    internal sealed class LoadPsFilter : IDisposable, IFilterImageProvider, IPICASuiteDataProvider
    {
...
#if GDIPLUS
        internal unsafe LoadPsFilter(Bitmap sourceImage, Color primary, Color secondary, Region selection, IntPtr owner)
#else
        internal unsafe LoadPsFilter(BitmapSource sourceImage, System.Windows.Media.Color primary, System.Windows.Media.Color secondary, Region selection, IntPtr owner)
#endif
        {
            if (sourceImage == null)
            {
        ...
        private unsafe LoadPsFilter(IntPtr owner)
        {
            parentWindowHandle = owner;
        ...
        private unsafe void SaveParameterHandles()
        {
            ...
            if (filterGlobalData != IntPtr.Zero)
            {
                ...
                {
                    int ps = HandleSuite.Instance.GetHandleSize(filterGlobalData);
                    byte[] dataBuf = new byte[ps];

                    Marshal.Copy(HandleSuite.Instance.LockHandle(filterGlobalData, 0), dataBuf, 0, dataBuf.Length);
                    HandleSuite.Instance.UnlockHandle(filterGlobalData);
            ...
            if (pluginDataBytes != null)
            {
                ...
                switch (globalParameters.PluginDataStorageMethod)
                {
                    case GlobalParameters.DataStorageMethod.HandleSuite:
                        ...
                        filterGlobalData = HandleSuite.Instance.NewHandle(pluginDataBytes.Length);
                        if (filterGlobalData == IntPtr.Zero)
                        {
                            throw new OutOfMemoryException(Resources.OutOfMemoryError);
                        }

                        Marshal.Copy(pluginDataBytes, 0, HandleSuite.Instance.LockHandle(filterGlobalData, 0), pluginDataBytes.Length);
                        HandleSuite.Instance.UnlockHandle(filterGlobalData);
                        break;
                    case GlobalParameters.DataStorageMethod.OTOFHandle:
        ...
        private bool PluginParameters()
        {
            ...
            unsafe
            {
                ...
                DebugUtils.Ping(DebugFlags.Call, string.Format("data: 0x{0},  parameters: 0x{1}", filterGlobalData.ToHexString(), filterRecord->parameters.ToHexString()));
            }
        ...
        {
#if DEBUG
            DebugUtils.Ping(DebugFlags.ColorServices, info != null ? string.Format("selector: {0}",  info->selector) : "<null>");
#endif
            ...
            switch (info->selector)
            {
                ...
                case ColorServicesSelector.SamplePoint:
                    ...
                    if (point->h >= 0 && point->h < source.Width && point->v >= 0 && point->v < source.Height)
                    {
                        ...
                        err = ColorServicesConvert.Convert(sourceSpace, info->resultSpace, info->colorComponents);
                    }
                    else
                    {
        ...
        private unsafe short DisplayPixelsProc(PSPixelMap* srcPixelMap, VRect* srcRect, int dstRow, int dstCol, IntPtr platformContext)
        {
            ...
            DebugUtils.Ping(DebugFlags.DisplayPixels, string.Format(
                "source = [ {0} ], srcRect = {1} dstCol (x, width) = {2}, dstRow (y, height) = {3}",
                new object[] { DebugUtils.PointerToString(srcPixelMap), DebugUtils.PointerToString(srcRect), dstCol, dstRow }));
#endif
            if (srcPixelMap == null || srcRect == null || platformContext == IntPtr.Zero)
            {
                return PSError.paramErr;
            }

            if (srcPixelMap->rowBytes == 0 || srcPixelMap->baseAddr == IntPtr.Zero ||
        ...
        private unsafe void Dispose(bool disposing)
        {
            if (!disposed)
            {
                ...
                if (filterGlobalData != IntPtr.Zero)
                {
                    ...
                    {
                        HandleSuite.Instance.UnlockHandle(filterGlobalData);
                        HandleSuite.Instance.DisposeHandle(filterGlobalData);
                    }
                    filterGlobalData = IntPtr.Zero;
                    pluginDataHandle = IntPtr.Zero;
                }
