        ...
        private void DrawGradient(Graphics g)
        {
            ...
            if (this.customGradient != null && gradientRect.Width > 1 && gradientRect.Height > 1)
            {
                using (Bitmap gradientSurface = new Bitmap(gradientRect.Width, gradientRect.Height, PixelFormat.Format24bppRgb))
                {
                    ...
                    {
                        Utility.DrawColorRectangle(gr, bitmapRect, Color.Transparent, false);
                    }
                    ...
                    if (Orientation == Orientation.Horizontal)
                    {
                            ...
                            unsafe
                            {
                                ...
                                for (int x = 0; x < gradientSurface.Width; ++x)
                                {
                                    ...
                                    for (int y = 0; y < gradientSurface.Height; ++y)
                                    {
                                        ...
                                        int rd = ((rp * ap) + (pixel[2] * (255 - ap))) / 255;
                                        int gd = ((gp * ap) + (pixel[1] * (255 - ap))) / 255;
                                        int bd = ((bp * ap) + (pixel[0] * (255 - ap))) / 255;
                        ...
                        {
                            gradientSurface.UnlockBits(data);
                        }

                        g.DrawImage(gradientSurface, gradientRect, bitmapRect, GraphicsUnit.Pixel);
                    }
            ...
            {
                using (LinearGradientBrush lgb = new LinearGradientBrush(this.ClientRectangle, maxColor, minColor, gradientAngle, false))
                {
